...
int* unlist(int (*jagged[])[], unsigned* jaggedsizes, unsigned* rows, unsigned nrows, unsigned* outlength){
  ...
  *outlength = count;
  ...
  count = 0;
  for(unsigned i=0; i<nrows; i++){
    //int row[jaggedsizes[rows[i]]] = (*jagged[rows[i]]); //[jaggedsizes[rows[i]]];
...
int main(){
  ...
  printf("---\n");
  printf("*test unlist*\n");
  unsigned rows[3] = {18,19,20};
  unsigned outlength;
  unsigned* unlisted = unlist(Faces, FacesSizes, rows, 3, &outlength);
