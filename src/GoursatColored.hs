...
funColor :: Triangle -> Double -> Color4 GLfloat
funColor triangle dmax = colors !! j
  where
...
voxel a b = makeVoxel (fGoursat a b)
            ((-2.5,2.5),(-2.5,2.5),(-2.5,2.5))
            (200, 200, 200)

trianglesGoursat :: Double -> Double -> Double -> IO ([Triangle], Double)
trianglesGoursat a b l = do
  vxl <- voxel a b
  computeContour3d''' vxl Nothing l
...
display rot1 rot2 rot3 a b l zoom = do
  ...
  b' <- get b
  l' <- get l
  (triangles, dist2max) <- trianglesGoursat a' b' l'
  loadIdentity
  resize z size
  rotate r1 $ Vector3 1 0 0
  rotate r2 $ Vector3 0 1 0
  rotate r3 $ Vector3 0 0 1
  renderPrimitive Triangles $ mapM_ (drawTriangle dist2max) triangles
  swapBuffers
  where
    drawTriangle dmax triangle = do
      let ((v1,v2,v3), norm) = fromTriangle triangle
      materialDiffuse FrontAndBack $= funColor triangle dmax
      normal norm
      vertex v1
      vertex v2
      vertex v3
